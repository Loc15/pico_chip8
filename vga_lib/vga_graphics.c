...
void clear_screen(void){
    //se borra la data del array del vga
    memset(&vga_data_array[0], false, sizeof(vga_data_array));  
}

void draw_display(const bool *display) {

    uint8_t color = BLACK;

    uint8_t scale = 4;

    uint16_t offset = 32 + (640*50);
    ...
    for(int i = 0; i < 64*32*scale*scale; i++){
      ...
      uint32_t pixel = ((i>>8)%2);

      uint32_t value = ((i>>2)%64);
      value = value + ((i>>10)*64);

      uint32_t pos = (i%(scale*64))+((i>>8)*640);

      //TOCA PORQUE AL ESTAR EN BOOLEANO ME BORRA LOS BITS DE WHITE
      if(display[value]){
      ...
      if (pixel & 1) {
          vga_data_array[pos + offset] = (vga_data_array[pos] & TOPMASK) | ((color) << 3) ;
      }
      else {
          vga_data_array[pos + offset] = (vga_data_array[pos] & BOTTOMMASK) | (color) ;
      }
...
/*desplazar la posicion afecta lo que se ve en pantalla*/
/*el offset es x + (y*ancho_display)*/